...
def StrainAlongTheta(x, y):
    ...
    ax[0].scatter(offsets/i, max_angles)
    ax[0].set_title("Angles Producing Maximum Tensile Strain vs. Tunnel Offset")
    ax[0].set_xlabel("Offset from Tunnel Axis (y/i)")
    ...
    ax[1].scatter(offsets/i, max_strain_for_offset*1e6)
    ax[1].set_title("Maximum Tensile Strain at Each Tunnel Offset")
    ax[1].set_xlabel("Offset from Tunnel Axis (y/i)")
    ...
    ax[2].scatter(offsets/i, max_corresponding_x/i)
    ax[2].set_title("Corresponding X-Coordinate for Each Tunnel Offset")
    ax[2].set_xlabel("Offset from Tunnel Axis (y/i)")
    ax[2].set_ylabel("X-Coordinate (x/i)")
    ...
    with PdfPages('Max Strain per Angle.pdf') as pdf:
        for it1 in range(len(wall_angle)):
            ...
            ax[0].scatter(offsets/i, max_strain_at_angle[:,it1]*1e6)
